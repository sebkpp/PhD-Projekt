...
def test_player_join(client):
    resp = client.post("/participants/join", json={"player_id": "1"})
...
def test_player_join_invalid(client):
    resp = client.post("/participants/join", json={"player_id": "99"})
    assert resp.status_code == status.HTTP_400_BAD_REQUEST


def test_player_heartbeat_after_join(client):
    client.post("/participants/join", json={"player_id": "1"})
    resp = client.post("/participants/heartbeat", json={"player_id": "1"})
    assert resp.status_code == status.HTTP_200_OK
    assert resp.json()["status"] == "heartbeat"


def test_player_heartbeat_invalid(client):
    resp = client.post("/participants/heartbeat", json={"player_id": "99"})
    assert resp.status_code == status.HTTP_400_BAD_REQUEST


def test_connection_status(client):
    resp = client.get("/participants/connection_status")
    assert resp.status_code == status.HTTP_200_OK
    data = resp.json()
...
def test_readiness_status_set_and_get(client):
    resp_set = client.post("/participants/readiness_status", json={
        "slot": "surgeon",
        "ready": True
    })
    ...
    resp_get = client.get("/participants/readiness_status")
...
def test_get_participants_by_experiment(client, experiment_id):
    resp = client.get(f"/participants/experiment/{experiment_id}")
    assert resp.status_code == status.HTTP_200_OK
...
def test_submit_participant(client, experiment_id, participant_id):
    ...
    resp = client.post("/participants/submit", json={
...
def test_get_submission_status(client, experiment_id, participant_id):
    ...
    client.post(f"/experiments/{experiment_id}/trials", json=trial_payload)
    client.post("/participants/submit", json={
        "experiment_id": experiment_id,
        "slot": 1,
        "participant_id": participant_id
    })

    resp = client.get(f"/participants/status/{experiment_id}/1")
    assert resp.status_code == status.HTTP_200_OK
