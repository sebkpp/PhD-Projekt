...
const SLOTS = [{ slot: "1", n: 1 }, { slot: "2", n: 2 }]
    ...
    return (
        <div className="flex flex-col gap-3">
            {SLOTS.map(({ slot, n }) => {
                ...
                return (
                    <div key={slot} className={`flex items-center justify-between rounded-lg px-4 py-3 border transition-colors ${
                        ready ? 'border-green-700 bg-green-900/20' : 'border-gray-700 bg-gray-800/50'
                    }`}>
                        <div className="flex items-center gap-2">
                            <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${ready ? 'bg-green-400' : 'bg-gray-500'}`} />
                            <span className="text-sm font-medium">{t('readiness.participant', { n })}</span>
                        ...
                        <button
                            ...
                            className={`text-xs px-3 py-1.5 rounded font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
                                ready ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-green-700 hover:bg-green-600 text-white'
                            }`}
                        >
                            {ready ? t('readiness.removeReady') : t('readiness.setReady')}
                        </button>
                    </div>
                )
            })}
