    ...
    public class CustomHandVisualizer : MonoBehaviour
    {
        ...
        protected void Update()
        {
            ...
            if (m_LeftHandGameObjects == null)
            {
                m_LeftHandGameObjects = new HandGameObjects(
                    ...
                    m_DebugDrawPrefab,
                    m_VelocityPrefab);
                LeftHand = m_LeftHandGameObjects.HandRoot;
            }
            ...
            if (m_RightHandGameObjects == null)
            {
                m_RightHandGameObjects = new HandGameObjects(
                    ...
                    m_DebugDrawPrefab,
                    m_VelocityPrefab);
                RightHand = m_RightHandGameObjects.HandRoot;
            }
        ...
        class HandGameObjects
        {
                ...
                GameObject velocityPrefab)
            {
                ...
                HandRoot = isSceneObject ? meshPrefab : Instantiate(meshPrefab, parent);
                HandRoot.SetActive(false); // Deactivate so that added components do not run OnEnable before they are finished being set up

                HandRoot.transform.localPosition = Vector3.zero;
                HandRoot.transform.localRotation = Quaternion.identity;

                var handEvents = HandRoot.GetComponent<XRHandTrackingEvents>();
                if (handEvents == null)
                {
                    handEvents = HandRoot.AddComponent<XRHandTrackingEvents>();
                    handEvents.updateType = XRHandTrackingEvents.UpdateTypes.Dynamic;
                    handEvents.handedness = handedness;
                }

                // m_MeshController = HandRoot.GetComponent<XRHandMeshController>();
                // if (m_MeshController == null)
                // {
                //     m_MeshController = HandRoot.AddComponent<XRHandMeshController>();
                //     for (var childIndex = 0; childIndex < HandRoot.transform.childCount; ++childIndex)
                //     {
                //         var childTransform = HandRoot.transform.GetChild(childIndex);
                ...
                var skeletonDriver = HandRoot.GetComponent<XRHandSkeletonDriver>();
                if (skeletonDriver == null)
                {
                    skeletonDriver = HandRoot.AddComponent<XRHandSkeletonDriver>();
